...
def pong_task(model):
    pe = PongEnv()
    sleep_duration_in_ms = np.random.normal(mu, sigma, 1)
    sleep(float(sleep_duration_in_ms)/1000)
    reward = 0
    return reward
